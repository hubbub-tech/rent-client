...
const PricingCard = ({ cart, isReady, toggle, setToggle, setFlashMessages, checkoutSession }) => {
  useEffect(() => setToggle(!toggle), []);
  return (
    <div className="card">
      <div className="card-body">
        <p className="text-start fs-5 fw-bold mb-3">Pricing</p>
        {isReady && <p className="text-start"><span className="badge bg-dark">Order Cost</span> { printMoney(cart.total_charge) }</p>}
        ...
        {isReady && <hr/>}
        {isReady && <p className="text-start"><span className="badge bg-dark">Total</span> { printMoney(cart.total_charge + cart.total_deposit + cart.total_tax) }</p>}
        <hr/>
        {isReady && <CheckoutForm setFlashMessages={setFlashMessages} checkoutSession={checkoutSession} />}
